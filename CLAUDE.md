...
Princípio de produto que filtra todo escopo: **"isso aumenta venda?"**. O CRM responde a três perguntas — _o que eu faço hoje?_ (vendedor), _quanto vamos fechar este mês?_ (gestor), _o que funciona pra vender?_ (método). Feature que não serve a nenhuma, corta.
...
- **Idempotência pelo UUID, nunca pelo título:** dedup por `deals.calendly_event_uid` (Calendly) e `meetingId` (tl;dv). O título/`event_type` só decide _quais_ eventos puxar.
...
- Server Components por padrão; Client Component só com interatividade (drag-and-drop do Kanban, botão _Atualizar_, forms).
